	...
	for _, nsServer := range nsServers {
		...
		if err != nil {
			...
			config.Tests = append(config.Tests, Test{
				TestID:   fmt.Sprintf("%s-soa-%s", zone, nsServers[0].IPv4[0]),
				ZoneName: zone,
		...
		if err == nil && len(nsRecords) > 0 {
			config.Tests = append(config.Tests, Test{
				TestID:   fmt.Sprintf("%s-ns-%s", zone, nsServers[0].IPv4[0]),
				ZoneName: zone,
		...
		if err == nil && len(aRecords) > 0 {
			config.Tests = append(config.Tests, Test{
				TestID:   fmt.Sprintf("%s-a-%s", zone, nsServers[0].IPv4[0]),
				ZoneName: zone,
				ServerIP: nsServers[0].IPv4[0],
				Interval: interval,
				TestType: "a",
		...
		if err == nil && len(aaaaRecords) > 0 {
			config.Tests = append(config.Tests, Test{
				TestID:   fmt.Sprintf("%s-aaaa-%s", zone, nsServers[0].IPv4[0]),
				ZoneName: zone,
				ServerIP: nsServers[0].IPv4[0],
				Interval: interval,
				TestType: "aaaa",
		...
		if err == nil && len(mxRecords) > 0 {
			config.Tests = append(config.Tests, Test{
				TestID:   fmt.Sprintf("%s-mx-%s", zone, nsServers[0].IPv4[0]),
				ZoneName: zone,
				ServerIP: nsServers[0].IPv4[0],
				Interval: interval,
				TestType: "mx",
